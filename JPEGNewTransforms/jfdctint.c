 ...
 * @param2: block column start
 * @param3: width index to  whole image matrix
 * @param4: height index to  whole image matrix
 * @param5: type of component 1-Y, 2-Cb, 3-Cr
 * return:
 */
void insertData(JSAMPARRAY sample_data, JDIMENSION start_col,int w,int h,int type){
  insertIntoList(type,w,h);
  if(type==0){//Y
    for(int i=0;i<8;i++){
      for(int j=0;j<8;j++){
        matImg[h+i][w+j]=sample_data[i][j+start_col]-shifting;
      }
    }
  }
  else if(type==1){//Cb
    for(int i=0;i<8;i++){
      for(int j=0;j<8;j++){
        matCHBImg[h+i][w+j]=sample_data[i][j+start_col]-shifting;
      }
    }
  }
  else if(type==2){//Cr
    for(int i=0;i<8;i++){
      for(int j=0;j<8;j++){
        matCHRImg[h+i][w+j]=sample_data[i][j+start_col]-shifting;
      }
    }
  }
}
...
if(transV==1){
  ...
  int j=0;
  int length=8;
  if(enable==1){//precalculated DCT basis
    for(i=0;i<8;i++){
      for(j=0;j<8;j++){
    ...
    for(j=0;j<8;j++){
      ...
      for(n1=0;n1<length;n1++){
        for(n2=0;n2<length;n2++){
          sum=sum+(sample_data[n1][n2+start_col]-128)*0.25*li*lj*fastMatrix[i][n1]*fastMatrix[j][n2];
        }
      }
      sum=sum*8;
      data[8*i+j]=(DCTELEM)sum;//write coefficients
      sum=0;
    }
  }
}
//********************Discrete sine transform*************
  ...
  if(enable==1){
    for(i=0;i<8;i++){
      for(j=0;j<8;j++){
        fastMatrix[i][j]=sin(((i+0.5)*PI*(j+1.0))/(8.5));
      }
    }
    enable=0;
  }
  for(i=0;i<8;i++){
    for(j=0;j<8;j++){
      for(n1=0;n1<length;n1++){
        for(n2=0;n2<length;n2++){
          sum=sum+(sample_data[n1][n2+start_col]-128)*fastMatrix[i][n1]*fastMatrix[j][n2];
        }
      }  
      sum=sum*sqrt(2.0/8.5)*sqrt(2.0/8.5);
      sum=sum*(dctScale[i][j]/dstScale[i][j]);
      sum=sum*8;
      data[8*i+j]=(DCTELEM)sum;
      sum=0;
    }
  }
}
//********************Walsh-Hadamard transform*************
  ...
  for(i=0;i<8;i++){
    for(j=0;j<8;j++){
      for(n1=0;n1<length;n1++){
        for(n2=0;n2<length;n2++){
          sum=sum+(sample_data[n1][n2+start_col]-128)*walsh[n1][i]*walsh[n2][j];
        }
      }
      tempsum=((double)sum)*(dctScale[i][j]/whtScale[i][j]);
      data[8*i+j]=(DCTELEM)tempsum;
      sum=0;
    }
  }
}
...
//********************Discrete Hartley transform*************
  ...
  if(enable==1){
    for(int i=0;i<8;i++){
      for(int j=0;j<8;j++){
        fastMatrix[i][j]=cos((2.0*PI*i*j)/((double)length))+sin((2.0*PI*i*j)/((double)length));
      }
    }
    double pom[8];
    for(int i=0;i<8;i++){
      for(int j=0;j<8;j++){
        pom[j]=fastMatrix[j][i]; 
      }
      fastMatrix[2][i]=pom[7];
      fastMatrix[3][i]=pom[2];
      fastMatrix[4][i]=pom[6];
      fastMatrix[6][i]=pom[3];
      fastMatrix[7][i]=pom[4];
    }
    enable=0;
  }
  ...
  for(int p=0;p<4;p++){
    ...
    for(int k1=0;k1<8;k1++){
      for(int k2=0;k2<8;k2++){
        if(isUnique(k1,k2,p,&rt,&ct)==1){
          for(int n1=0;n1<length;n1++){
            for(int n2=0;n2<length;n2++){
              sum=sum+getA(k1,k2,n1,n2,p)*(sample_data[n1][n2+start_col]-128);
            }
          }
          M[rt][ct]=sum;
          sum=0.0;
...
//********************Discrete Chebyshev transform*************
  ...
  for(i=0;i<8;i++){
    for(j=0;j<8;j++){
      ...
      for(n1=0;n1<length;n1++){
        for(n2=0;n2<length;n2++){
          sum=sum+(sample_data[n1][n2+start_col]-128)*DCHTMatrix[i][n1]*DCHTMatrix[j][n2];
        }
      }
      sum=sum*8;
      data[8*i+j]=(DCTELEM)sum;
...
//Local discrete cosine transform
else if(((transV>=4)&&(transV<=9))||(transV==11)){
  
  if(transV==11) shifting=0;
  if(enableWAV==1){
    HList=initHead();
    enableWAV=0;
  }
  if(load_matrix==1){
    //********loading whole image***********
    if(nstate==0){
      insertData(sample_data,start_col,wpos,hpos,0);
      if((wpos+8)>=widthImg){
        if((hpos+8)>=heightImg){
          nstate=4;
          wpos=0;
      ...
      insertData(sample_data,start_col,wpos+8,hpos,0);
      if((hpos+8)>=heightImg){
        if((wpos+16)>=widthImg){
          wpos=0;
          hpos=hpos+16;
      ...
      insertData(sample_data,start_col,wpos,hpos+8,0);
      if((wpos+8)>=widthImg){
        nstate=4;
        wpos=0;
        hpos=hpos+16;
      ...
      insertData(sample_data,start_col,wpos+8,hpos+8,0);
      nstate=4;
      if((wpos+16)>=widthImg){
        wpos=0;
        hpos=hpos+16;
      ...
      insertData(sample_data,start_col,wCHpos,hCHpos,1);
      nstate=5;
    }
    else if(nstate==5){
      insertData(sample_data,start_col,wCHpos,hCHpos,2);
      nstate=0;
      if((wCHpos+8)>=widthCHImg){
        wCHpos=0;
        hCHpos=hCHpos+8;
      }
      else{
  ...
  if(load_matrix==2){
    if(enable==1){
      HList->act=HList->first;
      //********************separable CDF 5/3*************
      if(transV==4){
        
        wavPass(matImg,0,0,widthImg,heightImg,1);
        wavPass(matImg,0,0,widthImg,heightImg,2);
        wavPass(matImg,0,0,widthImg,heightImg,4);

        wavPass(matCHBImg,0,0,widthCHImg,heightCHImg,1);
        wavPass(matCHBImg,0,0,widthCHImg,heightCHImg,2);
        wavPass(matCHBImg,0,0,widthCHImg,heightCHImg,4);
 
        wavPass(matCHRImg,0,0,widthCHImg,heightCHImg,1);
        wavPass(matCHRImg,0,0,widthCHImg,heightCHImg,2);
        wavPass(matCHRImg,0,0,widthCHImg,heightCHImg,4);
      }
      //********************separable CDF 9/7*************
      else if(transV==5){
        wavPass97(matImg,0,0,widthImg,heightImg,1);
        wavPass97(matImg,0,0,widthImg,heightImg,2);
        wavPass97(matImg,0,0,widthImg,heightImg,4);

        wavPass97(matCHBImg,0,0,widthCHImg,heightCHImg,1);
        wavPass97(matCHBImg,0,0,widthCHImg,heightCHImg,2);
        wavPass97(matCHBImg,0,0,widthCHImg,heightCHImg,4);
 
        wavPass97(matCHRImg,0,0,widthCHImg,heightCHImg,1);
        wavPass97(matCHRImg,0,0,widthCHImg,heightCHImg,2);
        wavPass97(matCHRImg,0,0,widthCHImg,heightCHImg,4);
      }
      //********************nonseparable CDF 5/3*************
      else if(transV==6){
        wavPassRB(matImg,widthImg,heightImg,1);
        wavPassRB(matImg,widthImg,heightImg,2);
        wavPassRB(matImg,widthImg,heightImg,4);
        
        wavPassRB(matCHBImg,widthCHImg,heightCHImg,1);
        wavPassRB(matCHBImg,widthCHImg,heightCHImg,2);
        wavPassRB(matCHBImg,widthCHImg,heightCHImg,4);
 
        wavPassRB(matCHRImg,widthCHImg,heightCHImg,1);
        wavPassRB(matCHRImg,widthCHImg,heightCHImg,2);
        wavPassRB(matCHRImg,widthCHImg,heightCHImg,4);
      }
      //********************nonseparable CDF 9/7*************
      else if(transV==7){
        wavPass97RB(matImg,widthImg,heightImg,1);
        wavPass97RB(matImg,widthImg,heightImg,2);
        wavPass97RB(matImg,widthImg,heightImg,4);

        wavPass97RB(matCHBImg,widthCHImg,heightCHImg,1);
        wavPass97RB(matCHBImg,widthCHImg,heightCHImg,2);
        wavPass97RB(matCHBImg,widthCHImg,heightCHImg,4);
 
        wavPass97RB(matCHRImg,widthCHImg,heightCHImg,1);
        wavPass97RB(matCHRImg,widthCHImg,heightCHImg,2);
        wavPass97RB(matCHRImg,widthCHImg,heightCHImg,4);
      }
      //********************modificated CDF 5/3*************
      else if(transV==8){
        for(int i=0;i<21;i++){
          wavPass(matImg,wavIndex[i][0],wavIndex[i][1],widthImg,heightImg,wavIndex[i][2]);
          wavPass(matCHBImg,wavIndex[i][0],wavIndex[i][1],widthCHImg,heightCHImg,wavIndex[i][2]);
          wavPass(matCHRImg,wavIndex[i][0],wavIndex[i][1],widthCHImg,heightCHImg,wavIndex[i][2]);
        }
      }
      //********************modificated CDF 9/7*************
      else if(transV==9){
        for(int i=0;i<21;i++){
          wavPass97(matImg,wavIndex[i][0],wavIndex[i][1],widthImg,heightImg,wavIndex[i][2]);
          wavPass97(matCHBImg,wavIndex[i][0],wavIndex[i][1],widthCHImg,heightCHImg,wavIndex[i][2]);
          wavPass97(matCHRImg,wavIndex[i][0],wavIndex[i][1],widthCHImg,heightCHImg,wavIndex[i][2]);
        }
      ...
      //********************local discrete cosine transform*************
        ...
        localdct(matImg,widthImg,heightImg);
        DCT_II(matImg,widthImg,heightImg);

        localdct(matCHBImg,widthCHImg,heightCHImg);
        DCT_II(matCHBImg,widthCHImg,heightCHImg);

        localdct(matCHRImg,widthCHImg,heightCHImg);
        DCT_II(matCHRImg,widthCHImg,heightCHImg);

      }
      enable=0;
    }
    //actual index offset
    int w=HList->act->wpos;
    int h=HList->act->hpos;
    double tempMat[8][8];
    double precis=8;
    if(HList->act->type==0){//write Y
      for(int i=0;i<8;i++){
        for(int j=0;j<8;j++){
          tempMat[i][j]=matImg[i+h][j+w]*precis;
        }
      }
      if((transV>=4)&&(transV<=9)) prepMat(tempMat);//index reversal
      if(transV==11){//LDCT scale low frequency
        for(int i=0;i<3;i++){
          for(int j=0;j<3;j++){
            tempMat[i][j]=tempMat[i][j]*LDCT_scale;
          }
        }
      }
      for(int i=0;i<8;i++){
        for(int j=0;j<8;j++){
          data[8*i+j]=(DCTELEM)tempMat[i][j];
        }
      }
    }
    else if(HList->act->type==1){//write Cb
      for(int i=0;i<8;i++){
        for(int j=0;j<8;j++){
          tempMat[i][j]=matCHBImg[i+h][j+w]*precis;
        }
      }
      if((transV>=4)&&(transV<=9)) prepMat(tempMat);//index reversal
      if(transV==11){//LDCT scale low frequency
      ...
      for(int i=0;i<8;i++){
        for(int j=0;j<8;j++){
          tempMat[i][j]=matCHRImg[i+h][j+w]*precis;
        }
      }
      if((transV>=4)&&(transV<=9)) prepMat(tempMat);//index reversal
      if(transV==11){//LDCT scale low frequency
    ...
    //scaling low frequency (wavelets)
    if((transV==4)||(transV==8)){//scale separable and modified CDF 5/3  
      for(int i=0;i<2;i++){
        for(int j=0;j<2;j++){
          data[8*i+j]=tempMat[i][j]*(dctScale[0][0]/2431.52);
        }
      }
    }
    if((transV==5)||(transV==9)){//scale separable and modified CDF 9/7
      for(int i=0;i<2;i++){
        for(int j=0;j<2;j++){
          data[8*i+j]=tempMat[i][j]*(dctScale[0][0]/1665.27);
        }
      }
    }
    if(transV==6){//scale nonseparable CDF 5/3
      for(int i=0;i<2;i++){
        for(int j=0;j<2;j++){
          data[8*i+j]=tempMat[i][j]*(dctScale[0][0]/4031.46);
        }
      }
    }
    if(transV==7){//scale nonseparable CDF 9/7
      for(int i=0;i<2;i++){
        for(int j=0;j<2;j++){
          data[8*i+j]=tempMat[i][j]*(dctScale[0][0]/2382.58);
        }
      }
    }
    //destroy whole image
    HList->act=HList->act->next;
    if(HList->act==NULL){
      
      struct listMem *pomL;
      HList->act=HList->first;
      while(HList->act!=NULL){
        pomL=HList->act;
        HList->act=HList->act->next;
        free(pomL);
      }
