...
main (int argc, char **argv)
{
...
#endif
  //****************cycle for whole image transforms*************
  //1.cycle: load whole image
  //2.cycle: process whole image
  do{
  if(load_matrix==1)load_matrix=2;
    int file_index;
   ...
   */
  if(load_matrix!=1){// 1.cycle: skip write 
  ...
  if (output_file != stdout)
    fclose(output_file);
  if(load_matrix==2)load_matrix=0;
}while(load_matrix!=0);
#ifdef PROGRESS_REPORT
